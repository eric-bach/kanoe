...
import { CustomResource, Duration, Stack, custom_resources } from 'aws-cdk-lib';
...
export class BedrockAgent extends Construct {
  ...
  constructor(scope: Construct, id: string, props: BedrockAgentProps) {
    ...
    this.bedrockAgentCustomResourceRole.addToPolicy(
      new PolicyStatement({
    ...
    this.actionGroups.forEach((actionGroup) =>
      actionGroup.actionGroupExecutor != 'Undefined' ? this.lambdaAttachResourceBasedPolicy(actionGroup.actionGroupExecutor) : 'Undefined'
    );
    ...
    const onEvent = new PythonFunction(this, 'BedrockAgentCustomResourceFunction', {
      ...
      entry: path.join(__dirname, '..', '..', 'src', 'bedrock_custom_resource'),
